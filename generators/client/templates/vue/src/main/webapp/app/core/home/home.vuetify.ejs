<template>
    <div class="home layout row wrap jh-card">
        <div class="flex md3">
            <span class="hipster img-fluid rounded"></span>
        </div>
        <div class="flex md9">
            <h1 class="display-2" v-text="$t('home.title')">Welcome, Java Hipster!</h1>
            <p class="lead" v-text="$t('home.subtitle')">This is your homepage</p>

            <div>
                <div class="alert alert-success success" v-if="authenticated">
                    <span v-if="username" v-text="$t('home.logged.message', { 'username': username})">You are logged in as user "{{username}}"</span>
                </div>

                <div class="alert alert-warning elevation-1 pa-4 mb-4 amber lighten-4" v-if="!authenticated">
